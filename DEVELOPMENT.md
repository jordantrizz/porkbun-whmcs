...
- GitHub Actions workflow [.github/workflows/secret-scan.yml](.github/workflows/secret-scan.yml) runs [gitleaks](https://github.com/gitleaks/gitleaks) on every push to `main`, every pull request, and a weekly schedule.
- The scan covers full git history, so a leaked value introduced and later removed is still caught.
...
- If gitleaks flags a confirmed false positive, add a narrowly scoped allowlist in a `.gitleaks.toml` rather than disabling the rule.
